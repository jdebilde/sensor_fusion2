...
calibrate() ->
    io:format("nav2_acc_gyro (acc): Calibrating... Do not move the pmod_nav!!~n"),
    [Ax,Ay,Az] = calibrate(acc, [out_x_xl, out_y_xl, out_z_xl], 300),
    io:format("nav2_acc_gyro (acc): ~p,~p,~p [g]~n", [Ax,Ay,Az]),
    io:format("nav2_acc_gyro (gyro): Place de pmod_nav flat and still!~n"),
    [Gx,Gy,Gz] = calibrate(acc, [out_x_g,out_y_g,out_z_g], 300),
    io:format("nav2_acc_gyro (gyro): ~p,~p,~p [deg/s]~n", [Gx,Gy,Gz]),
